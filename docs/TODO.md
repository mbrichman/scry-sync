...
- **ChatGPT support — follow-ups to the read-only PoC** (v2.6.0 shipped the Chrome adapter + export page)
  ...
  - **Image + file bytes** — the PoC renders image parts as placeholders. Full-fidelity capture needs resolving ChatGPT `sediment://` asset pointers via `GET backend-api/files/download/:id` (returns a signed `download_url`, then fetch the bytes), plus code-interpreter images in `metadata.aggregate_result.messages[].image_url`, and bundling them (the Claude adapter's `collectAllFiles` analog). Endpoint/scheme confirmed against pionxzh/chatgpt-exporter `src/api.ts`.
