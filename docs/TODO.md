...
- **ChatGPT support — follow-ups to the read-only PoC** (v2.6.0 shipped the Chrome adapter + export page)
  ...
  - **Auth hardening** — `/api/auth/session` token can expire mid-run; add refresh/retry. 429 now throws `ChatGptRateLimitError` (with `retryAfterMs`); a bulk export should catch it and back off/resume rather than aborting.
