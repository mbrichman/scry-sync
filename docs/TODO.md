...
- **ChatGPT support — follow-ups to the read-only PoC** (v2.6.0 shipped the Chrome adapter + export page)
  - **Wired into the real sync pipeline — DONE (v2.8.0 continuous ChatGPT sync via the `SOURCES` registry; v2.9.0 one shared sync core for dashboard, page and engine).** Phase 3 DONE (v3.0.0): one dashboard with source tabs, ChatGPT page retired, Claude off the tab relay, sub-status restored. Remaining cleanup: drop `jszip.min.js` once `background.js` stops injecting it; remove the unreferenced export renderers from `chatgpt_adapter.js`. Projects column removal confirmed by the owner. Backend status: Scry's `/api/conversations/ingest` accepts a verbatim `mapping` body and prunes server-side (scry PR #46, live) — the manual *Push selected → Scry* on the PoC page uses it (v2.7.0/2.7.1). Server-side `reconcile` is source-agnostic and is what the ChatGPT continuous sync relies on; `verify-deletable` is deliberately Claude-only (no ChatGPT delete until it can prove a `mapping` capture).
  - **Image bytes — DONE (v2.6.1).** `sediment://` pointers resolve via `GET backend-api/files/download/:id` → signed URL → bytes → inline data URI, for both multimodal parts and code-interpreter `aggregate_result` images. Remaining: **original non-image file bytes** (PDFs, docs, binaries the user uploaded) — no `content_type` carries these in the visible branch the way Claude's `document_asset` does; needs investigation of ChatGPT's attachment metadata (`message.metadata.attachments`) before it can be the `collectAllFiles` analog. **Push sends image bytes as `files[]` (v2.7.2)** — keyed by asset id, whole tree; the export path still inlines them as data URIs by design.
  - **Team / workspace accounts** — reads need a `Chatgpt-Account-Id` header derived from `backend-api/accounts/check/v4-2023-04-27` + the `_account` cookie. The adapter already plumbs the header through, but the off-origin PoC page can't read that cookie via `document.cookie`; auto-detection needs a content script on chatgpt.com (or a cookies-permission read). Personal accounts work with the token alone.
  - **Auth hardening** — `/api/auth/session` token can expire mid-run; add refresh/retry. 429 throws `ChatGptRateLimitError` (with `retryAfterMs`); the Scry push loop backs off and retries via `withChatGptRateLimitRetry` (v2.7.1) — the ZIP export paths do not yet.
  - **Richer rendering (optional)** — chatgpt-exporter also handles web-search citations/footnotes, LaTeX normalization, thinking/reasoning `<details>` blocks, and merging official-continuation nodes. Out of scope for a sync adapter, but noted if the export path grows.
