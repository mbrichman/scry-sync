...
- **ChatGPT support — follow-ups to the read-only PoC** (v2.6.0 shipped the Chrome adapter + export page)
  ...
  - **Wire into the real sync pipeline** — refactor the engine (`continuous_sync.js` / `scry_sync.js` / `scry_client.js`) to be multi-source, add a `chatgpt` adapter path alongside `claude`, and map `normalizeChatGptConversation` output onto `buildIngestPayload`. Backend status: Scry's `/api/conversations/ingest` accepts a verbatim `mapping` body and prunes server-side (scry PR #46, live) — the manual *Push selected → Scry* on the PoC page uses it (v2.7.0/2.7.1). Still missing server-side: `reconcile` / `verify-deletable` for `source_type: 'chatgpt'` (verify-deletable is deliberately Claude-only).
