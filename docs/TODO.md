...
- **ChatGPT support — follow-ups to the read-only PoC** (v2.6.0 shipped the Chrome adapter + export page)
  - **Wired into the real sync pipeline — DONE (v2.8.0 continuous ChatGPT sync via the `SOURCES` registry; v2.9.0 one shared sync core for dashboard, page and engine).** Remaining: fold the ChatGPT page into the dashboard with source tabs (phase 3); restore the per-image / rate-limit sub-status via an `onStatus` hook. Backend status: Scry's `/api/conversations/ingest` accepts a verbatim `mapping` body and prunes server-side (scry PR #46, live) — the manual *Push selected → Scry* on the PoC page uses it (v2.7.0/2.7.1). Still missing server-side: `reconcile` / `verify-deletable` for `source_type: 'chatgpt'` (verify-deletable is deliberately Claude-only).
